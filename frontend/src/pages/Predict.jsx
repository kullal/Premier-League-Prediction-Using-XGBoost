import React, { useState, useEffect } from "react";
  ...
  function Predict() {
    ...
    // <-- CHANGED: use public assets path (place files in frontend/public/assets/...)
    const getTeamLogo = (teamName) => {
      const base = import.meta.env.BASE_URL || "/"; // works with Vite
      const publicAssetsBase = `${base}assets/team-logos`;
      if (!teamName) return `${base}assets/Premier-League-Logo-White.png`;
      const safe = teamName.replace(/\s+/g, "-").toLowerCase();
      return `${publicAssetsBase}/${encodeURIComponent(safe)}.png`;
    };
    ...
    const handlePredict = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/predict/history?home_team=${homeTeam}&away_team=${awayTeam}`
        );
    ...
    return (
      <div className="relative min-h-screen bg-black text-white overflow-visible">
        <div className="absolute w-full h-full">
          <img
            src="/src/assets/nighttime-serenity-a-3d-rendering_13271352.jpg"
            alt=""
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
              ...
              <div className="bg-black/30 border border-white/6 rounded-xl p-6 flex flex-col min-h-[24rem]">
                ...
                >
                  ...
                  <Menu.Items className="absolute left-0 mt-2 w-full max-h-56 overflow-y-auto bg-white/5 border border-white/10 rounded-lg z-50">
                    {teams.map((team) => (
                      <Menu.Item key={team}>
                        {({ active }) => (
                          <button
                ...
                <div className="flex-1 flex items-center justify-center bg-black/40 border border-white/6 rounded-lg p-4">
                  <img
                    ...
                    onError={(e) => {
                      ...
                      e.currentTarget.src = `${import.meta.env.BASE_URL || "/"}assets/Premier-League-Logo-White.png`;
                    }}
